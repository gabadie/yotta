#include "yotta_dictate_daemon_start.private.h"
...
typedef struct
yotta_dictate_daemon_start_cmd_s
{
...
yotta_dictate_daemon_start_cmd_t;
...
static
void
yotta_dictate_daemon_start_release(yotta_tcp_cmd_t * cmd)
{
    yotta_free(cmd);
}
...
static
void
yotta_dictate_daemon_start_send(yotta_dictate_daemon_start_cmd_t * cmd)
{
...
)
{
     ...
     */
    yotta_dictate_daemon_start_cmd_t * cmd = yotta_alloc_s(yotta_dictate_daemon_start_cmd_t);

    yotta_dirty_s(cmd);

    yotta_tcp_cmd_init(cmd);
    yotta_tcp_cmd_set_send(cmd, yotta_dictate_daemon_start_send);
    yotta_tcp_cmd_set_release(cmd, yotta_dictate_daemon_start_release);

    cmd->header_cursor = 0;
    cmd->header.label = YOTTA_DICTATE_LABEL_DAEMON_START;
